...
module.exports = class extends EventEmitter {
  ...
  get (url) {
    ...
    return this.fetcher.get(url).then((response) => {
      ...
      this.emit('response', response.url, response);
      var triples = [], prefixes = {}, promise = q.defer();
      const extn1 = url.split('.').pop()
      const extn2 = response.url.split('.').pop()
      const responseType = response.type.toLowerCase()
      function match(...types) {
        for (const type of types) {
          if (responseType === type) return true
          if (responseType === 'text/plain') {
            const data = mimeDB[type]
            if (data.extensions.includes(extn1)) return true
            if (data.extensions.includes(extn2)) return true
          }
        }
        return false
      }
      if (match('application/ld+json', 'application/json')) {
      ...
      } else if (match("application/trig","application/n-quads","text/turtle","application/n-triples","text/n3")) {
      ...
      } else if (match("application/rdf+xml")) {
        //RDFXML
